...
namespace SIL.PrepFLExDB
{
	...
	public partial class ChooseLangProjectDialog : Form
	{
		...
		private ChooseLangProjectDialog()
		{
			...
			m_linkOpenBridgeProject.Visible = false; //// File.Exists(FLExBridgeHelper.FullFieldWorksBridgePath());
			pictureBox1.Visible = false; //// File.Exists(FLExBridgeHelper.FullFieldWorksBridgePath());
		}
		...
		/// </param>
		/// ------------------------------------------------------------------------------------
		public ChooseLangProjectDialog(bool openToAssosiateFwProject)
			: this()
		{
		...
		private void HandleProjectFindingExceptions(Exception e)
		{
			...
			{
				MessageBox.Show(ActiveForm, e.Message, "Warning", MessageBoxButtons.OK,
		...
		private void OpenFwDataProjectLinkClicked(object sender, LinkLabelLinkClickedEventArgs e)
		{
			...
			using (var dlg = new OpenFileDialog())
			{
				...
				dlg.Filter = "FieldWorks (*.fwdata)|*.fwdata"; //// ResourceHelper.FileFilter(FileFilterType.FieldWorksProjectFiles);
