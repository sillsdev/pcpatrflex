...
namespace SIL.DisambiguateInFLExDB
{
	public class ToneParsLogConverter
	{
		...
		public string ConvertHvosToMorphnames()
		{
			...
			var logMiddleWithGlosses = hvoGlossMapper.Aggregate(LogFileMiddle, (current, replacement) => current.Replace(replacement.Key, replacement.Value));
		...
		private void BuildHvoGlossMapper(Dictionary<string, string> hvoGlossMapper)
		{
			...
			var matches = Regex.Matches(LogFileContents, " Working on: ([^\r\n]+)", RegexOptions.Multiline);
			foreach (Match match in matches)
			{
				...
				foreach (string sHvo in hvos)
				{
					...
					if (Int32.TryParse(sHvo, out hvo))
					{
						...
						if (msa != null)
						{
							var sense = msa.ReferringObjects.FirstOrDefault(item => item is ILexSense) as ILexSense;
							if (sense != null)
							{
