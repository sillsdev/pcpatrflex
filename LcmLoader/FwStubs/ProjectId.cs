...
namespace SIL.LcmLoader
{
	...
	public class ProjectId : ISerializable, IProjectIdentifier
	{
		...
		{
			get { return "MyXYZ" /*FwUtils.GeneratePipeHandle(Handle)*/; }
		}
		...
		{
			get { return PipeHandle.Substring(0/*FwUtils.ksSuiteName.Length + 1*/); }
		}
		...
		public bool IsValid
		{
			get
			{
				...
				if (ex == null)
					return true;
				if (ex is Exception /*StartupException*/)
					return false;
		...
		internal Exception GetExceptionIfInvalid()
		{
			if (string.IsNullOrEmpty(Name))
				return new Exception("No Project Name");//// StartupException(Properties.Resources.kstidNoProjectName, false);
			...
			switch (Type)
			{
				...
				case BackendProviderType.kSharedXML:
					if (!FileUtils.SimilarFileExists(Path))
						return new Exception("File Not Found:'" + Path + "'"); ////StartupException(string.Format(Properties.Resources.kstidFileNotFound, Path));
					break;
				case BackendProviderType.kInvalid:
					return new Exception("Invalid Fw Project Type"); ////StartupException(Properties.Resources.kstidInvalidFwProjType);
				default:
